                ...
                <div class="card-header pb-0 card-no-border">
                    <div class="d-flex justify-content-between flex-sm-row flex-column">
                        <div class="pb-3 pb-md-0">
                            <h4 class="title">{{ __('profession_list') }} ({{ count($professions) }})</h4>
                        </div>
                        <div>
                            <div class="d-flex flex-row">
                                <button class="btn btn-primary" type="button" data-bs-toggle="modal"
                                    data-bs-target="#tooltipmodal"><i class="fa fa-plus"> </i> {{ __('bulk_import') }}
                                </button>
                            </div>
                        </div>
                    </div>
                                ...
                                <tr>
                                    ...
                                    <td>
                                        <div class="d-flex align-items-center">
                                            ...
                                            <a href="{{ route('profession.edit', $profession->id) }}" class="btn">
        ...
        <div class="col-md-4">
            ...
            <div class="card">
                <div class="card-header">
                    <h4>{{ __('create') }} {{ __('profession') }}</h4>
                </div>
                <div class="card-body">

                    @if (userCan('job_role.create'))
                    <form class="row g-3" action="{{ route('profession.store') }}" method="POST"
                        ...
                        @endphp
                        <div class="col-md-12">
                            <label class="form-label" name="$label" for="name" @required(true)>{{ __($label) }}<spna
                                    class="text-red"> * </spna>
                            </label>
            ...
            <div class="card">
                <div class="card-header">
                    <div class="float-start">
                        <h4>{{ __('edit') }} {{ __('profession') }}</h4>
                    </div>
                    <div class="float-end">
                        <a href="{{ route('profession.index') }}" class="btn btn-primary"><i class="fa fa-plus"></i>{{
                            __('create') }}
                        </a>
                    </div>
                </div>
                <div class="card-body">
                    <form class="row g-3" action="{{ route('profession.update', $prof->id) }}" method="POST">
                        @method('PUT')
                        @csrf
                        @foreach ($app_language as $key => $language)
                        @php
    ...
    <div class="modal fade" id="tooltipmodal" tabindex="-1" role="dialog" aria-labelledby="tooltipmodal"
        ...
        <div class="modal-dialog modal-dialog-centered" role="document">
            <div class="modal-content">
                ...
                <form action="{{ route('admin.profession.bulk.import') }}" method="post" enctype="multipart/form-data">
                    @csrf
