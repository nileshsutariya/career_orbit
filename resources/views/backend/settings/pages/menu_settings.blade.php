                            ...
                            <tr data-id="{{ $menu->id }}">
                                ...
                                <td class="vertical-middle" tabindex="0">
                                    ...
                                    <form id="statusChangeForm{{ $menu->id }}"
                                        action="{{ route('menu-setting.status.change', $menu->id) }}" method="POST">
                                        @csrf
                                ...
                                <td>
                                    <div class="d-flex">
                                        ...
                                        @if (!$menu->default)
                                        <form action="{{ route('menu-settings.destroy', $menu->id) }}" class="d-inline"
                                            method="POST">
                                            @method('DELETE')
                                            @csrf
    ...
    <div class="col-md-4">
        ...
        <div class="card">
            <div class="card-header">
                ...
                <div class="float-end">
                    <a href="{{ route('menu-settings.index') }}" class="btn btn-primary">
            ...
            <div class="card-body">
                @if (userCan('menu-setting.update'))
                <form action="{{ route('menu-settings.update', $menu_setting->id) }}" method="POST">
                    @method('PUT')
                    @csrf
                    ...
                    __('title') .
                    ' ' .
                    getLanguageByCodeInLookUp($language->code, $app_languages);
                    ...
                    <div class="col-md-12 mb-2">
                        ...
                        <label class="form-label" name="$label" for="title" @required(true)>
                            {{ $label }} <span class="text-danger"> * </span> </label>
                        <input id="title" type="text" name="{{ $title }}" placeholder="{{ __('title') }}"
                            value="{{ $value }}" class="form-control @if ($errors->has($title)) is-invalid @endif">
                        @if ($errors->has($title))
                    ...
                    <div class="col-md-12 mb-2">
                        ...
                        <label class="form-label" name="url" for="url" @required(true)>
                            {{ __('url') }} <span class="text-danger">*</span></label>
                    ...
                    <div class="col-md-12 mb-2">
                        ...
                        <label class="form-label"> {{ __('visibility_this_menu') }} <span
                                class="text-danger">*</span></label>
                        <div>
                            <!-- Checkboxes for "everyone", "employer", and "candidate" -->
                            <label>
                                <input type="checkbox" name="eligibility[]" value="public" class="hidden-checkbox"
                                    multiple>
                                <span class="select-button edit {{ in_array('public', $arrayValue) ? 'selected' : '' }}"
                                    data-value="public">{{ __('everyone') }}</span>
                            </label>
                            <label>
                                <input type="checkbox" name="eligibility[]" value="employee" class="hidden-checkbox"
                                    multiple>
                                <span
                            ...
                            <label>
                                <input type="checkbox" name="eligibility[]" value="candidate" class="hidden-checkbox"
                                    multiple>
                                <span
                    ...
                    <div class="col-md-12">
                        <label class="form-label" for="status">
                            {{ __('show_in_the_menu') }}</label>
                        <div class="form-check-size">
                           
                            <div class="form-check form-switch form-check-inline">
                                <input class="form-check-input switch-primary check-size success status-switch"
                                    value="1" @checked($menu_setting->status) name="status"
                                type="checkbox">
                            </div>
            ...
            <div class="card-body">
                ...
                <form action="{{ route('menu-settings.store') }}" method="POST">
                    ...
                    <div class="col-md-12 mb-2">
                        ...
                        <label class="form-label" for="title" @required(true)>
                            {{ $label }} <span class="text-danger">*</span></label>
                        <input id="title" type="text" name="{{ $title }}" placeholder="{{ __('title') }}"
                            value="{{ old($title) }}" class="form-control @if ($errors->has($title)) is-invalid @endif">
                        @if ($errors->has($title))
                        <span class="invalid-feedback" role="alert">
                            <strong>{{ $errors->first($title) }}</strong>
                        </span>
                    ...
                    <div class="col-md-12 mb-2">
                        ...
                        <label for="url" class="form-label" @required(true)>{{ __('url') }} <span class="text-danger">*
                            </span></label>
                        <input id="url" type="text" name="url" placeholder="{{ __('enter') }} {{ __('url') }}"
                    ...
                    <div class="mb-2">
                        ...
                        <label for="eligibility" class="form-label" @required(true)>{{ __('visibility_this_menu') }}
                    ...
                    <div class="col-md-12">
                        ...
                        {{-- <div>
                            <label class="switch">
                                <input value="1" name="status" type="checkbox" class="success status-switch">
                        ...
                        <div class="form-check-size">
                            <div class="form-check form-switch form-check-inline">
                                <input class="form-check-input switch-primary check-size success status-switch"
                                    type="checkbox" role="switch" value="1" name="status">
                            </div>
                        </div>
