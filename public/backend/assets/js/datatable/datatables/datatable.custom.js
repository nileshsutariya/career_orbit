(function ($) {
  $(document).ready(function () {
    ...
    $("#advance-8").DataTable({
      language: {
        url: "{{Asset('admin/assets/json/German.json')}}",
      },
    });
    $("#advance-9").DataTable({
      searching: false,
    ...
    $("#data-source-2").DataTable({
      ajax: "{{Asset('admin/assets/ajax/arrays.txt')}}",
    });
    var dataSet = [
      [
        "Tiger Nixon",
    ...
    $("#data-source-4").DataTable({
      ...
      ajax: "{{Asset('admin/assets/json/server-side.json')}}",
    });
    ...
    var ct = $("#API-chield-row").DataTable({
      ajax: "{{Asset('admin/assets/ajax/api.txt')}}",
      columns: [
        {
    ...
    $("#ajax-data-object").DataTable({
      ajax: "{{Asset('admin/assets/ajax/object.txt')}}",
      columns: [
        {
          data: "name",
        },
    ...
    $("#ajax-data-nested-object").DataTable({
      processing: true,
      ajax: "{{Asset('admin/assets/ajax/object_nested.txt')}}",
      columns: [
        {
          data: "name",
        },
    ...
    $("#orthogonal-data").DataTable({
      ajax: "{{Asset('admin/assets/ajax/orthogonal.txt')}}",
      columns: [
        {
          data: "name",
        },
    ...
    $("#server-side-datatable").DataTable({
      processing: true,
      serverSide: true,
      ajax: "{{Asset('admin/assets/ajax/server-processing.php')}}",
    });
    ...
    $("#datatable-http").DataTable({
      ...
      serverSide: true,
      ajax: {
        url: "{{Asset('admin/assets/ajax/server-processing.php')}}",
        data: function (d) {
          d.myKey = "myValue";
        },
      },
    ...
    $("#datatable-post").DataTable({
      ...
      serverSide: true,
      ajax: {
        url: "{{Asset('admin/assets/ajax/post.php')}}",
        type: "post",
      },
      columns: [
        {
